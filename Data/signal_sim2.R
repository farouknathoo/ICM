...
index1 <- 192
c1 <- which( Dist[index1,] < 40)

which(sub.vert$x < -20 & sub.vert$y< -20 & sub.vert$z < -20)
index2 <- 166
c2 <- which( Dist[index2,] < 28)

plot3d(sub.vert[-c(c1,c2),],pch=19,col="red")
points3d(sub.vert[c1,],pch=19,col="black")  
points3d(sub.vert[c2,],pch=19,col="blue")  
play3d( spin3d(rpm=3), duration=15)

t <- 1:T
s1 <- 10*dnorm(t,mean = 70,sd = 10)
plot(t,s1,type='l')

s2 <- 10*dnorm(t,mean = 150,sd = 10)
