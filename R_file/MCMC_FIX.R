...
S.sum <- matrix(0, nrow = dim(S)[1],ncol = dim(S)[2])
S.sum.sq <- matrix(0, nrow = dim(S)[1],ncol = dim(S)[2])
...
for (r in 1:R ) {
  ...
  Z.nv[,r] <- cube.state #STORE UPDATED STATES FROM THIS SWEEP
