...
## Replacing PL with full conditional for Z and apply chequerboard updating. 
...
S.power <- apply(S, 1, var)
Z.state <- rep(0,P)
vert.cluster <- kmeans(S,K)
c1.index <- which(vert.cluster$cluster==1)
c2.index <- which(vert.cluster$cluster==2)
c3.index <- which(vert.cluster$cluster==3)
c1.power <- mean(S.power[c1.index])
c2.power <- mean(S.power[c2.index])
c3.power <- mean(S.power[c3.index])

power.rank <- rank(c(c1.power,c2.power,c3.power))
Z.state[c1.index] <- power.rank[1]
Z.state[c2.index] <- power.rank[2]
Z.state[c3.index] <- power.rank[3]

table(Z.state)
plot3d(sub.vert,col = Z.state,pch=19)

cube.state <- rep(0,n.v)
# empty voxels are assigned to be in state 1 
v.index.nz <- as.numeric(names(table(vert.Z)))
cube.state[-v.index.nz] <- 1
...
{
  idx <- which(vert.Z==v) 
  max.location <- which.is.max(table(Z.state[idx]))
  cube.state[v] <- as.numeric( names(table(Z.state[idx]))[max.location])
}
...
R  <- 150 #NUMBER OF ICM ITERATIONS TO RUN
...
icm_S <- S
icm_beta <- beta_star[R]
icm_sigma2_M <- sigma2_M_star[R]
icm_sigma2_E <- sigma2_E_star[R]
icm_sigma2_a <- sigma2_a_star[R]
icm_A <- matrix(vec_A_star[,R],nrow = K-1)
icm_mu <- mu_star[,,R]
icm_alpha <- alpha_star[,R]
icm_cube_state <- Z.nv[,R]
icm_Z.state  <- Z.state

save(icm_S,file = "./Data/icm_S.RData")  
save(icm_beta,file = "./Data/icm_beta.RData")  
save(icm_sigma2_a,file = "./Data/icm_sigma2_a.RData")  
save(icm_alpha,file = "./Data/icm_alpha.RData")  
save(icm_A, file = "./Data/icm_A.RData")
save(icm_mu, file = "./Data/icm_mu.RData")
save(icm_sigma2_M, file = "./Data/icm_sigma2_M.RData")
save(icm_sigma2_E,file = "./Data/icm_sigma2_E.RData")
save(icm_cube_state,file = "./Data/icm_cube_state.RData")
save(icm_Z.state,file = "./Data/icm_Z.state.RData")
